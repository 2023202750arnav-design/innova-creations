...
function Header() {
  ...
  const cartCount = useShop((s) => s.cart.reduce((count, item) => count + item.qty, 0));
  ...
  return <header className="sticky top-0 z-40">
    ...
    <div className="glass px-4 py-3">
      <div className="mx-auto flex max-w-7xl items-center gap-4">
        ...
        <Link aria-label="Cart" to="/cart" className="relative"><ShoppingBag /><Badge count={cartCount} /></Link>
      </div>
    </div>
...
function ProductCard({ product }: { product: Product }) {
  ...
  const isWishlisted = useShop((s) => s.wishlist.includes(product.id));
  ...
  return <motion.article whileInView={{ opacity: 1, y: 0 }} initial={{ opacity: 0, y: 18 }} className="card-hover group overflow-hidden rounded-lg border border-gold/20 bg-white">
    ...
    <div className="p-4"><div className="flex justify-between gap-3"><h3 className="font-display text-xl"><Link to={`/products/${product.slug}`}>{product.name}</Link></h3><button aria-label="Toggle wishlist" onClick={() => toggle(product.id)} className={isWishlisted ? "text-burgundy" : ""}><Heart size={20} fill={isWishlisted ? "currentColor" : "none"} /></button></div><p className="mt-1 line-clamp-2 text-sm text-mid">{product.shortDescription}</p><p className="mt-2 text-sm text-gold-dark"><Stars value={product.rating} /> {product.rating} ({product.reviews})</p><div className="mt-2 flex items-end gap-2"><b className="text-lg">{inr.format(product.price)}</b><s className="text-sm text-mid">{inr.format(product.compareAtPrice)}</s></div><div className="mt-4 flex gap-2"><button onClick={() => { addCart(product); toast.success("Added to cart"); }} className="gold-btn flex-1 rounded px-4 py-2 font-semibold">Add to Cart</button><Link to={`/products/${product.slug}`} className="rounded border border-gold px-3 py-2 text-sm">Quick View</Link></div></div>
  </motion.article>;
}
